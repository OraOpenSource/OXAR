...
if [ "$OOS_APEX_CREATE_USER_YN" = "Y" ]; then
  ...
  echo exit | sqlplus sys/$OOS_ORACLE_PWD as sysdba @apex_create_workspace.sql $OOS_APEX_USER_WORKSPACE $OOS_ORACLE_USER_NAME
  ...
  echo exit | sqlplus sys/$OOS_ORACLE_PWD as sysdba @apex_create_user.sql $OOS_APEX_USER_WORKSPACE $OOS_ORACLE_USER_NAME $OOS_APEX_USER_NAME $OOS_APEX_USER_PASS
